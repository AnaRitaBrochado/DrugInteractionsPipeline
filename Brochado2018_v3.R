...
Multi_Species = c("EC","ST","PA")
Species_strains = list(c("Ecoli_BW","Ecoli_iAi1"),c("ST_LT2","ST_14028"),c("PA_01","PA_14"))
names(Species_strains) = Multi_Species
All_Bugs = as.vector(unlist(Species_strains))

col_antagonism = "#FF9A39"
col_synergy = "#04859D"
Bug_colors = list(c("#F8A100","#FFB836"),c("#770300","#C3110C"),c("#119E0A","#056000"))
names(Bug_colors) = Multi_Species
Species_colors = c("#F8A100","#C3110C","#119E0A")

source(paste0(here_path,"Aux_functions.R", collapse=NULL))

#get drug classes table: attribute table
Attr_table = read.table(paste0(Load_dir,"Drug_Classes.txt"),sep="\t",header = T)
colors = as.character(Attr_table$Color)
colors = paste0('#',colors)
Attr_table[match("Color",names(Attr_table))]=colors
...
source(paste0(here_path,"/Brochado_2018_Parser.R",collapse=NULL))

#=============== Get strong synergies & antagonisms =========New===========
...
for(b in 1:length(Interaction_Qs))
{
  ...
  if(b==1)
  {hit_classif_list = list()}
  hit_classif = as.character(bug_hits$hit_classif)
  hit_classif_list[[length(hit_classif_list)+1]] = table(hit_classif)
  
}
names(Bugs_hits) = names(Interaction_Qs)

hit_classif_list = as.data.frame(t(as.data.frame(hit_classif_list)))
names(hit_classif_list) = as.character(as.matrix((hit_classif_list[1,])))
hit_classif_list = hit_classif_list[seq(2,length(hit_classif_list[[1]]),by=2),]
row.names(hit_classif_list) = names(Bugs_hits)

hit_classif_list = as.data.frame(hit_classif_list)

hit_classif_table=as.data.frame(matrix(as.numeric(as.matrix(hit_classif_list)),ncol=3))
names(hit_classif_table) = names(hit_classif_list)
row.names(hit_classif_table) = row.names(hit_classif_list)
hit_classif_table = as.data.frame(rbind(hit_classif_table,sapply(hit_classif_table,FUN=sum)))
row.names(hit_classif_table)[length(row.names(hit_classif_table))] = "Totals"

#clean up
rm(bug_hits,bug_interactions,bug_interactions_sign,b,syn,antag,opposite_int,hit_classif_list)

#============== Restrict to reduced/complete range ==NEW=========
...
for(b in 1:length(P_values))
{
  ...
  min_wells = min(bug_pvals[grep(T,bug_pvals$AntagConcRange <= multpTest_cutoff),4])
  possible_antagonsim = length(bug_pvals[(bug_pvals$AntagPot >= min_wells),1])
  min_wells = min(bug_pvals[grep(T,bug_pvals$SynConcRange <= multpTest_cutoff),5])
  possible_synergy = length(bug_pvals[(bug_pvals$SynPot >= min_wells),1])
...
#============= Make drug pie =======
pdf(paste0(Out_dir,"Brochado2018_v3.pdf"),useDingbats=FALSE)

pie_table = table(as.character(Attr_table$Class))
pie_table = pie_table[match(as.character(unique(Attr_table$Class)),names(pie_table))]
pie_colors = as.character(Attr_table$Color)[match(names(pie_table),as.character(Attr_table$Class))]
pie(pie_table,col = pie_colors)

rm(pie_table,pie_colors)
...
for(sp in 1:length(Multi_Species))
{
  ...
  bug1_hits_syn = bug1_hits[grep("Syn",bug1_hits$direction),]
  interac_score = bug1_hits_syn$bug_Q1
  p_value = bug1_hits_syn$CompleteConcRange
  hit_classif = bug1_hits_syn$hit_classif
  interac_score[grep("reduced_specific",hit_classif)] = bug1_hits_syn[grep("reduced_specific",hit_classif),match("bug_Q1_reduced",names(bug1_hits_syn))]
  p_value[grep("reduced_specific",hit_classif)] = bug1_hits_syn[grep("reduced_specific",hit_classif),match("SynConcRange",names(bug1_hits_syn))]
  bug1_hits_syn_temp = as.data.frame(cbind(as.character(bug1_hits_syn$direction),interac_score,p_value,as.character(hit_classif)))
  row.names(bug1_hits_syn_temp) = row.names(bug1_hits_syn)
  bug1_hits_syn = bug1_hits_syn_temp
  rm(bug1_hits_syn_temp)
  names(bug1_hits_syn) = c("direction","interac_score","p_value","hit_classif")
  
  bug1_hits_antag = bug1_hits[grep("Antag",bug1_hits$direction),]
  interac_score = bug1_hits_antag$bug_Q3
  p_value = bug1_hits_antag$CompleteConcRange
  hit_classif = bug1_hits_antag$hit_classif
  interac_score[grep("reduced_specific",hit_classif)] = bug1_hits_antag[grep("reduced_specific",hit_classif),match("bug_Q3_reduced",names(bug1_hits_antag))]
  p_value[grep("reduced_specific",hit_classif)] = bug1_hits_antag[grep("reduced_specific",hit_classif),match("AntagConcRange",names(bug1_hits_antag))]
  bug1_hits_antag_temp = as.data.frame(cbind(as.character(bug1_hits_antag$direction),interac_score,p_value,as.character(hit_classif)))
  row.names(bug1_hits_antag_temp) = row.names(bug1_hits_antag)
  bug1_hits_antag = bug1_hits_antag_temp
  rm(bug1_hits_antag_temp)
  names(bug1_hits_antag) = c("direction","interac_score","p_value","hit_classif")
  
  bug1_hits_restruct = as.data.frame(rbind(bug1_hits_syn,bug1_hits_antag))
  
  bug2_hits_syn = bug2_hits[grep("Syn",bug2_hits$direction),]
  interac_score = bug2_hits_syn$bug_Q1
  p_value = bug2_hits_syn$CompleteConcRange
  hit_classif = bug2_hits_syn$hit_classif
  interac_score[grep("reduced_specific",hit_classif)] = bug2_hits_syn[grep("reduced_specific",hit_classif),match("bug_Q1_reduced",names(bug2_hits_syn))]
  p_value[grep("reduced_specific",hit_classif)] = bug2_hits_syn[grep("reduced_specific",hit_classif),match("SynConcRange",names(bug2_hits_syn))]
  bug2_hits_syn_temp = as.data.frame(cbind(as.character(bug2_hits_syn$direction),interac_score,p_value,as.character(hit_classif)))
  row.names(bug2_hits_syn_temp) = row.names(bug2_hits_syn)
  bug2_hits_syn = bug2_hits_syn_temp
  rm(bug2_hits_syn_temp)
  names(bug2_hits_syn) = c("direction","interac_score","p_value","hit_classif")
  
  bug2_hits_antag = bug2_hits[grep("Antag",bug2_hits$direction),]
  interac_score = bug2_hits_antag$bug_Q3
  p_value = bug2_hits_antag$CompleteConcRange
  hit_classif = bug2_hits_antag$hit_classif
  interac_score[grep("reduced_specific",hit_classif)] = bug2_hits_antag[grep("reduced_specific",hit_classif),match("bug_Q3_reduced",names(bug2_hits_antag))]
  p_value[grep("reduced_specific",hit_classif)] = bug2_hits_antag[grep("reduced_specific",hit_classif),match("AntagConcRange",names(bug2_hits_antag))]
  bug2_hits_antag_temp = as.data.frame(cbind(as.character(bug2_hits_antag$direction),interac_score,p_value,as.character(hit_classif)))
  row.names(bug2_hits_antag_temp) = row.names(bug2_hits_antag)
  bug2_hits_antag = bug2_hits_antag_temp
  rm(bug2_hits_antag_temp)
  names(bug2_hits_antag) = c("direction","interac_score","p_value","hit_classif")
  
  bug2_hits_restruct = as.data.frame(rbind(bug2_hits_syn,bug2_hits_antag))
  ...
  p_val_bug1 = rep(NA,length(all_hits))
  p_val_bug2 = rep(NA,length(all_hits))
  ...
  int_score_bug1[match(row.names(bug1_hits_restruct),all_hits)] = as.numeric(as.matrix(bug1_hits_restruct[[2]]))
  int_score_bug2[match(row.names(bug2_hits_restruct),all_hits)] = as.numeric(as.matrix(bug2_hits_restruct[[2]]))
  p_val_bug1[match(row.names(bug1_hits_restruct),all_hits)] = as.numeric(as.matrix(bug1_hits_restruct[[3]]))
  p_val_bug2[match(row.names(bug2_hits_restruct),all_hits)] = as.numeric(as.matrix(bug2_hits_restruct[[3]]))
  
  species_conserved = as.data.frame(cbind(dir_bug1,dir_bug2,int_score_bug1,int_score_bug2,p_val_bug1,p_val_bug2))
  ...
  #Find interaction scores for weak interactions
      #bug1
  ...
  species_conserved = species_conserved[c(1,2,7,3,4,5,6)]
...
#****************Check "SynergyEnrichWithin"********

All_probed = as.vector(unlist(lapply(Interaction_Qs,FUN=row.names)))

#Count all possible withihn and across Master_class
drug1 = unlist(strsplit(All_probed ,split="_"))[seq(1,2*length(All_probed ),by=2)]
drug2 = unlist(strsplit(All_probed ,split="_"))[seq(2,2*length(All_probed ),by=2)]
class1 = get_feature(drug1,feature = "Master_Class",Attr_table)
class2 = get_feature(drug2,feature = "Master_Class",Attr_table)

all_within = length(grep(T,class1==class2))
all_across = length(grep(F,class1==class2))

#Count synergies within and across Master_class
#all_syneries = Results_list$Interactions_table
all_syneries = Absolute_interactions
all_syneries = all_syneries[grep("Synergy",all_syneries$direction),]

all_syneries = as.character(all_syneries[[1]])

drug1 = unlist(strsplit(all_syneries ,split="_"))[seq(1,2*length(all_syneries),by=2)]
drug2 = unlist(strsplit(all_syneries ,split="_"))[seq(2,2*length(all_syneries),by=2)]
class1 = get_feature(drug1,feature = "Master_Class",Attr_table)
class2 = get_feature(drug2,feature = "Master_Class",Attr_table)

syn_within = length(grep(T,class1==class2))
syn_across = length(grep(F,class1==class2))

C_table = as.data.frame(rbind(c(syn_across,syn_within),
                              c(all_across-syn_across,all_within-syn_within)))
names(C_table) = c("Across","Within")
row.names(C_table) = c("Synergy","no Synergie")

#p_val = fisher.test(C_table)
p_val = chisq.test(C_table)
p_val = p_val$p.value

rm(drug1,drug2,class1,class2,syn_across,syn_within,
   all_across,all_within,all_syneries,All_probed,C_table)
...
Results_list = list(Abs_counts, #Total, Syn and Antag counts
                    ...
                    hit_classif_table, #Counts for complete&reduced range interactions
                    p_val) #Synergies enriched within drug master classes
...
names(Results_list) = c("Abs_counts","Abs_counts_summary","hit_rates_species",
                        ...
                        "nr_drugs_1int_per_bug",
                        "Interactions_table",
                        "Complete/Reduced range interactions",
                        "SynergyEnrichWithin")

#=============== Compare all bugs against each other ================
Unique_interacs = c()
for(b in 1:length(Interac_distrib))
{
  bug_interacs = names(Interac_distrib[[b]])
  Unique_interacs = c(Unique_interacs,bug_interacs)
}
Unique_interacs = unique(Unique_interacs)
for(b in 1:length(Interaction_Qs))
{
  bug = names(Interaction_Qs)[b]
  bug_interac =Interaction_Qs[[b]]
  bug_int_score = bug_interac$bug_int_score
  
  bug_data = rep(NA,length(Unique_interacs))
  bug_data[match(row.names(bug_interac),Unique_interacs)] = bug_int_score
  
  bug_data = as.data.frame(bug_data)
  row.names(bug_data) = Unique_interacs
  names(bug_data) = bug
  
  if(b==1)
  {All_bug_interactions = bug_data} else
  {All_bug_interactions = as.data.frame(cbind(All_bug_interactions,bug_data))}
}
my_colors_ind3 = colorRampPalette(c("white","grey"))
heatmap.2(cor(All_bug_interactions,use = "complete.obs"),trace="none",main="All strains correlation",col=my_colors_ind3,
          breaks=seq(0,1,by=0.1),cexRow = 0.8, cexCol = 0.8)
rm(my_colors_ind3)
#========= Get conserved interaction networks - classes ==========
for(sp in 1:length(Multi_Species))
{
  species = Multi_Species[sp]
  sp_hits = Spieces_conservation[[sp]]
  ...
  #********* for classes
  ClassesInter_Tables = get_Classes_simm_heatmap_new(species,complete_Hits = c(),Hits=Hits,Attr_table,Drug_classes="Class")
  
  p_val = chisq.test(ClassesInter_Tables$ContingencyTable[1:2,])$p.val
  p_val2 = fisher.test(ClassesInter_Tables$ContingencyTable[1:2,])$p.val
  text(x=0,y=-0.9,labels=paste0("Chi squared p-val = ",round(p_val,8)))
  text(x=0,y=-1,labels=paste0("Fisher's exact p-val = ",round(p_val2,8)))
  
  par(mfrow=c(1,1),mar=c(4,4,4,4))
  Classes = unique(as.character(Attr_table$Class))
  Class_colors = as.character(Attr_table[match(Classes,as.character(Attr_table$Class)),4])
  get_Classes_network(species,ClassesInter_Tables,Classes,Class_colors)
  
  tablePos = ClassesInter_Tables$ClassInterPos
  tableNeg = ClassesInter_Tables$ClassInterNeg
  
  Cytoscape_table = as.data.frame(cbind(rep(row.names(tablePos),length(tablePos[[1]])),rep(row.names(tablePos),each=length(tablePos[[1]])),as.vector(as.matrix(tablePos)),
                                        rep("Antagonism",length(tablePos[[1]]))))
  Cytoscape_table = as.data.frame(rbind(Cytoscape_table,
                                        as.data.frame(cbind(rep(row.names(tableNeg),length(tableNeg[[1]])),rep(row.names(tableNeg),each=length(tableNeg[[1]])),as.vector(as.matrix(tableNeg)),
                                                            rep("Synergy",length(tableNeg[[1]]))))
  ))
  
  names(Cytoscape_table) = c("Class1","Class2","NrInteractions","direction")
  Cytoscape_table = Cytoscape_table[-grep(0,Cytoscape_table$NrInteractions),]
  
  file_id = paste0(Out_dir,species,"_speciesInteracCytoscape.txt")
  write.table(Cytoscape_table,file_id,sep="\t",row.names=F,quote=FALSE,col.names = T)
  
  #********* for Master_Class
  ClassesInter_Tables = get_Classes_simm_heatmap_new(species,complete_Hits = c(),Hits=Hits,Attr_table,Drug_classes="Master_Class")
  p_val = chisq.test(ClassesInter_Tables$ContingencyTable)$p.val
  p_val2 = fisher.test(ClassesInter_Tables$ContingencyTable[1:2,])$p.val
  text(x=0,y=-0.9,labels=paste0("Chi squared p-val = ",round(p_val,8)))
  text(x=0,y=-1,labels=paste0("Fisher's exact p-val = ",round(p_val2,8)))
  
  par(mfrow=c(1,1),mar=c(4,4,4,4))
  Master_Classes = unique(as.character(Attr_table$Master_Class))
  MClass_colors = as.character(Attr_table[match(Master_Classes,as.character(Attr_table$Master_Class)),4])
  
  if(sp==1) {thick_factor = 0.2} else
    {thick_factor = 0.5}
  
  Cytoscape_table = get_Classes_network(species,ClassesInter_Tables,Master_Classes,MClass_colors,thick_factor = thick_factor,cytoscape_files = T)
  Cytoscape_table = Cytoscape_table[[1]]
  
  file_id = paste0(Out_dir,species,"_speciesInteracCytoscape_generalTarget.txt")
  write.table(Cytoscape_table,file_id,sep="\t",row.names=F,quote=FALSE,col.names = T)
}

#========= Get add_up monocrhomaticity plot for all strains - classes ==========

#Account for all interactions, not only conserved. n=2586
for(sp in 1:length(Multi_Species))
{
  species = Multi_Species[sp]
  sp_hits = Spieces_conservation[[sp]]
  ...
  sp_hits = as.data.frame(cbind(drug1,interaction,drug2,row.names(sp_hits)))
  names(sp_hits) = c("DrugA","Interaction","DrugB","DrugA_B")
  #Duplicate to have both strains
  hits = as.data.frame(rbind(sp_hits,sp_hits))
  
  #non conserved, not comparable and specific
  sp_hits = Spieces_conservation[[sp]]
  
  sp_hits = sp_hits[c(grep_exact("specific",sp_hits[[3]]),
                      grep_exact("not_comparable",sp_hits[[3]]),
                      grep_exact("not_conserved",sp_hits[[3]])),]
  
  bug1_ints = sp_hits[!is.na(as.vector(sp_hits[[1]])),]
  interaction1 = as.character(unlist(bug1_ints[[1]]))
  
  bug2_ints = sp_hits[!is.na(as.vector(sp_hits[[2]])),]
  interaction2 =  as.character(unlist(bug2_ints[[2]]))
  
  interaction = c(interaction1,interaction2)
  
  drug12 = c(row.names(bug1_ints),row.names(bug2_ints))
  drug1 = unlist(strsplit(drug12,split="_"))[seq(1,2*length(drug12),by=2)]
  drug2 = unlist(strsplit(drug12,split="_"))[seq(2,2*length(drug12),by=2)]
  
  non_cons_hits = as.data.frame(cbind(drug1,interaction,drug2,drug12))
  names(non_cons_hits) = c("DrugA","Interaction","DrugB","DrugA_B")
  
  hits = as.data.frame(rbind(hits,non_cons_hits))
  
  if(sp==1)
  {Hits=hits} else
  {
    Hits = as.data.frame(rbind(Hits,hits))
  }
}

species = "All species"
ClassesInter_Tables = get_Classes_simm_heatmap_new(species,complete_Hits = c(),
                                                   Hits=Hits,Attr_table,Drug_classes="Class",
                                                   plotting = F,plotting2 = F,reporting = F)

#For Monochromaticity plots
par(mfrow=c(1,1),mar=c(6,6,6,6))
all = ClassesInter_Tables$ClassInterNeg + ClassesInter_Tables$ClassInterPos
neg_fraction = ClassesInter_Tables$ClassInterNeg/(all)
pos_fraction = ClassesInter_Tables$ClassInterPos/(all)
pos_all = ClassesInter_Tables$ClassInterPos
pos_fraction[all==1]=NaN
neg_fraction[all==1]=NaN
pos_all[all==1]=0
all[all==1]=NaN

pos_all = sum(as.vector(pos_all))/sum(as.vector(all),na.rm=T)
MC_index = as.vector(as.matrix(pos_fraction))
MC_index[grep(T,MC_index<pos_all)] = ((MC_index-pos_all)/pos_all)[grep(T,MC_index<pos_all)]
MC_index[grep(T,MC_index>pos_all)] = ((MC_index-pos_all)/(1-pos_all))[grep(T,MC_index>pos_all)]
MC_index[grep(T,MC_index==pos_all)] = 0

mat_MC_index = as.data.frame(matrix(MC_index,length(pos_fraction)))
names(mat_MC_index) = names(pos_fraction)
row.names(mat_MC_index) = row.names(pos_fraction)

p=mat_MC_index
mat_MC_index[is.na(mat_MC_index)]=0
p[!is.na(p)]=0
p[is.na(p)]=1
par(mfrow=c(1,1),mar=c(6,6,6,6))
corrplot(as.matrix(mat_MC_index),method="color",type="lower",tl.col="black",tl.cex=0.6,cl.lim=c(-1,1),
         col=colorRampPalette(c(col_synergy,"grey90",col_antagonism))(10),p.mat=as.matrix(p),sig.level = 0.5,insig="blank",mar = c(1,1,1,1))
title(paste0(species," Monochromaticy index"),line=-3)

for(cl in 1:length(mat_MC_index))
{
  class = names(mat_MC_index)[cl]
  
  class_mono = mat_MC_index[[cl]]
  class_p = p[[cl]]
  
  class_mono_t = as.numeric(mat_MC_index[cl,])
  class_p_t = as.numeric(p[cl,])
  
  class_mono[1:cl] = class_mono_t[1:cl]
  class_p[1:cl] = class_p_t[1:cl]
  
  class_mono[grep(1,class_p)] = NA
  
  class_mono = as.data.frame(class_mono)
  names(class_mono) = class
  if(cl==1)
  {Classes_mono = class_mono} else
  {Classes_mono = as.data.frame(cbind(Classes_mono,class_mono))}
  
}

#****************** Plot classes separately ****************** 
#Add nr of interactions
count_negs = ClassesInter_Tables$ClassInterNeg + t(ClassesInter_Tables$ClassInterNeg) 
diagonal = diag(t(ClassesInter_Tables$ClassInterNeg))
for(i in 1:length(diagonal))
{
  count_negs[i,i] = count_negs[i,i]-diagonal[i]
}

count_pos = ClassesInter_Tables$ClassInterPos + t(ClassesInter_Tables$ClassInterPos) 
diagonal = diag(t(ClassesInter_Tables$ClassInterPos))
for(i in 1:length(diagonal))
{
  count_pos[i,i] = count_pos[i,i]-diagonal[i]
}
total_interactions_class = sapply((count_negs + count_pos),FUN=sum)
#reset par
par(mfrow = c(3,3),mar=c(2,2,2,2))
for(c in 1:length(Classes_mono))
{
  plot(density(Classes_mono[[c]],na.rm=T),col="grey",lwd=2,xlim=c(-1.5,1.5),ylim=c(0,0.8),
       main=names(Classes_mono)[c],axes=F,frame=T)
  text(x=-1.2,y=0.7,paste0("i=",total_interactions_class[c]),adj=0)
  n = length(grep_exact(names(total_interactions_class)[c],as.character(Attr_table[,2])))
...
#*********Conserved Synergies vs Antagonism *******
conserv_antag = length(grep("Antag",as.character(Conserved2sp_interac$interaction)))
conserv_syn = length(grep("Syn",as.character(Conserved2sp_interac$interaction)))
antagonism_overlap = length(grep("Antag",as.character(Overlaps$direction)))
syn_overlap = length(grep("Syn",as.character(Overlaps$direction)))
non_conserv_antag = antagonism_overlap - conserv_antag
non_conserv_syn = syn_overlap - conserv_syn

C_table = as.data.frame(t(matrix(c(conserv_antag,non_conserv_antag,conserv_syn,non_conserv_syn),ncol=2)))
row.names(C_table) = c("Antagonism","Synergy")
names(C_table) = c("Conserved","Non-conserved")

mosaicplot(C_table,main="Synergy conserved 2 sp")
sign = chisq.test(C_table)
sign2 = fisher.test(C_table)
text(x = 0.2,y=0,paste0("chisq p-val = ",round(sign$p.value,4)),adj=0)
text(x = 0.2,y=0.1,paste0("Fisher's p-val = ",round(sign2$p.value,4)),adj=0)

Conserved3sp_interac = Conserved2sp_interac[grep("fully",as.character(Conserved2sp_interac$conservation)),]
conserv_antag = length(grep("Antag",as.character(Conserved3sp_interac$interaction)))
conserv_syn = length(grep("Syn",as.character(Conserved3sp_interac$interaction)))
antagonism_overlap = length(grep("Antag",as.character(Overlaps$direction)))
syn_overlap = length(grep("Syn",as.character(Overlaps$direction)))
non_conserv_antag = antagonism_overlap - conserv_antag
non_conserv_syn = syn_overlap - conserv_syn
...
mosaicplot(C_table,main="Synergy conserved 3 sp")
sign = chisq.test(C_table)
sign2 = fisher.test(C_table)
text(x = 0.2,y=0,paste0("chisq p-val = ",round(sign$p.value,4)),adj=0)
text(x = 0.2,y=0.1,paste0("Fisher's p-val = ",round(sign2$p.value,4)),adj=0)
...
Species_comparison = c(length(Overlaps[[1]]),#Overlaps
                         length(Conflicts[[1]]),#Conflicts
                         length(Conserved3sp_interac[[1]]), #Fully conserved
                         length(Conserved2sp_interac[[1]]), #Partially conserved
                         length(Conserved3sp_interac[[1]])/length(Overlaps[[1]]), #fraction of fully conserved
                         length(Conserved3sp_interac[[1]])/(length(Overlaps[[1]])+length(Conflicts[[1]])), #fraction of fully conserved including overlaps
                         (length(Overlaps[[1]])-length(Conserved2sp_interac[[1]]))/length(Overlaps[[1]])#Species specific
                         )
Species_comparison = as.data.frame(Species_comparison)
row.names(Species_comparison) = c("Overlaps","Conflicts","Fully conserved","Partially conserved","fraction of fully conserved",
                                  "fraction of fully conserved including overlaps","Species specific")
#clean up
rm(conserv_antag,conserv_syn,non_conserv_antag,
   non_conserv_syn,antagonism_overlap,syn_overlap,sign,C_table,Conserved2sp_interac,Conserved3sp_interac)

#============== Update Results_list ============

Results_list[[length(Results_list)+1]] = Species_comparison
names(Results_list)[length(Results_list)]= "Species_comparison"

#Add nr possible interactions across drug categories and withihn drug categories
Classes = unique(as.character(Attr_table$Class))
within_combs = 0
for(cl in 1:length(Classes))
{
  class = Classes[cl]
  n_class_drugs = length(grep_exact(class,as.character(Attr_table$Class)))
  
  if(n_class_drugs>1)
  {
    n_combs = length(as.data.frame(combinations(n_class_drugs, 2))[[1]])
    within_combs = within_combs+n_combs
  }
  
}
all_combs = length(as.data.frame(combinations(length(as.character(Attr_table$Drug)), 2))[[1]])
across_combs = all_combs-within_combs

Across_versus_within = across_combs/within_combs
Results_list[[length(Results_list)+1]] = Across_versus_within
names(Results_list)[length(Results_list)]= "Combinations_across_versus_within_target"

#========== Source Supp Figures ======
par(mfrow = c(1,1),mar=c(6,6,6,6))
source(paste0(here_path,"/SuppFigs.R",collapse=NULL))
dev.off()

save.image(paste0(Load_dir,"Brochado2018.RData"))

#========== Source Benchamarking and clinical isolates ======
par(mfrow = c(1,1),mar=c(6,6,6,6))
source(paste0(here_path,"/Benchmarking&ClinicIsolates.R",collapse=NULL))

file_id = paste0(Out_dir,"BenchmarkingOut.txt")
write.table(Benchmarking,file_id,sep="\t",row.names=T,quote=FALSE,col.names = T)
